//#region
const inputTwo: string = `iwaduermzfyc
zdmrlfyau
wfdzhrmauy
yfzdmaru
dtagqzbovprnyufm
...
const sixArr = inputTwo.split(/\n\n/)

// const countedArr = sixArr.map((str) => {
//   const cleanedString = str.replace(/\n/g, "")
//   const stringToArr = cleanedString.split("")
//   const arrToSet = new Set(stringToArr)
//   return Array.from(arrToSet).length
// })

// const reducedArr = countedArr.reduce((num, val) => num + val, 0)

// console.log(reducedArr)

const catArr = sixArr.map((str) => {
  let numToReturn = 0
  const numOfLines = str.split(/\n/).length
  const cleanedString = str.replace(/\n/g, "")
  const charCounter = cleanedString.split("").reduce((counter, char) => {
    counter[char] = ++counter[char] || 1
    return counter
  }, {})
  for (const prop in charCounter) {
    if (charCounter[prop] === numOfLines) numToReturn++
  }
  return numToReturn
})

const sumArr = catArr.reduce((num, val) => num + val, 0)
console.log(sumArr)
