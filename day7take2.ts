// #region
const day7take2Input: string = `drab tan bags contain 4 clear gold bags.
...
type luggage = { [color: string]: Array<Array<string & number>> }

// splitting string to array
const day7arrTakeTwo: string[] = day7take2Input.split(/\n/)

// reducing array of strings to luggage type object
const luggageReduced: luggage = day7arrTakeTwo.reduce((luggage, str) => {
  // for each element of the array

  // splitting the string in two on the substring " bags contain ", which is present in every line
  const firstSplit: string[] = str.split(" bags contain ")

  // taking the second index of firstSplit, splitting it on the comma and looping through the resulting array and replacing the words "bags", "bag" and periods from each element, as well as trimming whitespace
  const bagColorAndNum: string[] = firstSplit[1]
    .split(",")
    .map((str) => str.replace(/bags?|\./g, "").trim())

  // looping through each value and changing it to a two element array consisting of a number parsed from the first character and a slice string started at the third index of the string
  const stringsToArrays = bagColorAndNum.map((str) => [+str[0], str.slice(2)])

  // if, and only if, the first value of the first array is truthy, we use bracket notation to assign as a key the first index of the first split value we made, remembering to trim, and them assigning the array of arrays we made to that
  if (stringsToArrays[0][0]) luggage[firstSplit[0].trim()] = stringsToArrays
  return luggage
}, {})

const countInnerBagsTwo = (bags: luggage, target: string): number => {
  // if the the key we are targeting in the luggage type object, does not exist, return 0
  if (!bags[target]) {
    return 0
  }
  let innerBags: number = 0
  //recursively call a whole bunch of this function until there is nothing left to call
  for (const innerBag of bags[target]) {
    // add innerBag[0] for number of current inner bag
    // plus innerBag[0] * the number of bags in the inner bag
    innerBags +=
      innerBag[0] + innerBag[0] * countInnerBagsTwo(bags, innerBag[1])
  }
  return innerBags
}

console.log(countInnerBagsTwo(luggageReduced, "shiny gold"))
