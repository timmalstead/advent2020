...
const day7Input: string = `drab tan bags contain 4 clear gold bags.
...
let day7arr = day7Input.split(/\n/)

const split2: Array<Array<string>> = day7arr.map((str) =>
  str.split(" bags contain ")
)
...
const split3: Array<Array<string>> = split2.map((arr) => {
  const arrToReturn = []
  arrToReturn.push(arr[0])
  const splitBags = arr[1]
    .split(",")
    .map((str) => str.replace(/bags?|\./g, "").trim())
  splitBags.forEach((bag) => arrToReturn.push(bag))
  return arrToReturn
})

const endsWithGold = (str): boolean => str.endsWith("shiny gold")

const split4: Array<Array<string>> = split3.filter((arr) => {
  return arr.some(endsWithGold)
  // && arr[0] !== "shiny gold"
})

let previousTotal: number = 0
let nextTotal: number = 1
let currentSplit = split4

while (previousTotal !== nextTotal) {
  previousTotal = nextTotal
  const colorSearch = currentSplit.map((arr) => arr[0]).join("|")
  const searchReg = new RegExp(`${colorSearch}`)
  const searchRegTest = (str) => searchReg.test(str)
  const newSplit = split3.filter((arr) => arr.some(searchRegTest))
  currentSplit = newSplit
  nextTotal = newSplit.length
}

const objWithArrs = currentSplit.reduce((obj, arr) => {
  const numsOfBags = arr.slice(1).reduce((arr, str) => {
    const arrToPush = []
...
for (const line of day7arr) {
  // split bag types after outer bag type
  let innerBagTypes: any[] = line.replace(/.*?bags/, "").split(",")
...
// #region
const layer1ColorSearch = split4.map((arr) => arr[0]).join("|")

const firstSearchReg = new RegExp(`${layer1ColorSearch}`)

const firstSearchRegTest = (str) => firstSearchReg.test(str)

const split5 = split3.filter((arr) => arr.some(firstSearchRegTest))
...
const layer2ColorSearch = split5.map((arr) => arr[0]).join("|")

const secondSearchReg: RegExp = new RegExp(`${layer2ColorSearch}`)

const secondSearchRegTest = (str) => secondSearchReg.test(str)

const split6 = split3.filter((arr) => arr.some(secondSearchRegTest))

const layer3ColorSearch = split6.map((arr) => arr[0]).join("|")

const thirdSearchReg: RegExp = new RegExp(`${layer3ColorSearch}`)

const thirdSearchRegTest = (str) => thirdSearchReg.test(str)

const split7 = split3.filter((arr) => arr.some(thirdSearchRegTest))

const layer4ColorSearch = split7.map((arr) => arr[0]).join("|")

const fourthSearchReg: RegExp = new RegExp(`${layer4ColorSearch}`)

const fourthSearchRegTest = (str) => fourthSearchReg.test(str)

const split8 = split3.filter((arr) => arr.some(fourthSearchRegTest))

const layer5ColorSearch = split8.map((arr) => arr[0]).join("|")

const fifthSearchReg: RegExp = new RegExp(`${layer5ColorSearch}`)

const fifthSearchRegTest = (str) => fifthSearchReg.test(str)

const split9 = split3.filter((arr) => arr.some(fifthSearchRegTest))

const layer6ColorSearch = split9.map((arr) => arr[0]).join("|")

const sixthSearchReg: RegExp = new RegExp(`${layer6ColorSearch}`)

const sixthSearchRegTest = (str) => sixthSearchReg.test(str)

const split10 = split3.filter((arr) => arr.some(sixthSearchRegTest))

const layer7ColorSearch = split10.map((arr) => arr[0]).join("|")

const seventhSearchReg: RegExp = new RegExp(`${layer7ColorSearch}`)

const seventhSearchRegTest = (str) => seventhSearchReg.test(str)

const split11 = split3.filter((arr) => arr.some(seventhSearchRegTest))

const layer8ColorSearch = split11.map((arr) => arr[0]).join("|")

const eigthSearchReg: RegExp = new RegExp(`${layer8ColorSearch}`)

const eigthSearchRegTest = (str) => eigthSearchReg.test(str)

const split12 = split3.filter((arr) => arr.some(eigthSearchRegTest))
